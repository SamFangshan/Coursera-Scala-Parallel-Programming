...
class KMeans extends KMeansInterface {
  ...
  def classify(points: Seq[Point], means: Seq[Point]): Map[Point, Seq[Point]] = {
    var meanToPointsInCluster = points.groupBy(findClosest(_, means))
    for (mean: Point <- means) {
      if (!(meanToPointsInCluster contains mean)) {
        meanToPointsInCluster += mean -> Seq[Point]()
      }
    }
    meanToPointsInCluster
  }
  ...
  def classify(points: ParSeq[Point], means: ParSeq[Point]): ParMap[Point, ParSeq[Point]] = {
    ...
    for (mean: Point <- means) {
      if (!(meanToPointsInCluster contains mean)) {
        meanToPointsInCluster += mean -> ParSeq[Point]()
      }
    }
    meanToPointsInCluster
  }
  ...
  def converged(eta: Double, oldMeans: Seq[Point], newMeans: Seq[Point]): Boolean = {
    !(0 until oldMeans.length).map(i => oldMeans(i).squareDistance(newMeans(i))).exists(_ > eta)
